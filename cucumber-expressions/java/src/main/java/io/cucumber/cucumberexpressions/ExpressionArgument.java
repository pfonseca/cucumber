...
public class ExpressionArgument<T> implements Argument<T> {
    ...
    static List<Argument<?>> build(TreeRegexp treeRegexp, List<ParameterType<?>> parameterTypes, String text) {
        ...
        for (int i = 0; i < parameterTypes.size(); i++) {
            ...
            args.add(new ExpressionArgument<>(argGroup, parameterType));
        }
    ...
    public ExpressionArgument(Group group, ParameterType<T> parameterType) {
        this.group = group;
